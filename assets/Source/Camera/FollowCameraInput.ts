import { _decorator, Camera, Component, EventMouse, EventTouch, Node, PhysicsSystem, toDegree, Vec2 } from 'cc';
...
export class FollowCameraInput extends Component {
    ...
    public AddKfc(key:string,func:Function){
        this._kfcList.set(key,func);
    }

    public RemoveKfc(key:string){
        if(!this._kfcList.has(key)){
            return;
        }
        this._kfcList.delete(key);
    }
    ...
    private _onTouchEnd (eventTouch: EventTouch) {
        ...
        //
        const ray = this.camera.node.getComponent(Camera)!.screenPointToRay(eventTouch.getLocationX(), eventTouch.getLocationY());
        const system = PhysicsSystem.instance;

        if (system.raycast(ray)) {
            const results = system.raycastResults;
            if (results.length > 0) {
                for(const result of results){
                    const hitNode = result.collider.node;
                    
                    const hitWorldPos = result.hitPoint;
                    const hitNormal = result.hitNormal;
                    const hitCollider = result.collider;
                    const hitDistance = result.distance;

                    // console.log("===== 射线碰撞结果 =====");
                    // console.log("碰撞物体名称：", hitNode.name);
                    // console.log("碰撞点世界坐标：", hitWorldPos.x, hitWorldPos.y, hitWorldPos.z);
                    // console.log("碰撞面法线：", hitNormal);
                    // console.log("碰撞距离：", hitDistance);

                    if(this._kfcList.has(hitNode.name)){
                        this._kfcList.get(hitNode.name).call(this._kfcList.get(hitNode.name),hitWorldPos);
                        break;
                    }
                }
            }
    ...
    private _kfcList:Map<string,Function> = new Map();
